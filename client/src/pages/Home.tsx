...
import { Plus, Users, X, Briefcase, LayoutGrid } from 'lucide-react'
...
export default function Home({ user }: { user: User }) {
  ...
  const handleMatch = async (offerId: string, amount: number) => {
    ...
    else { alert("Matched!"); fetchLobby(); fetchData(); }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        {activeTab === 'market' ? (
          <>
            ...
            <section className="mb-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {offers.map((offer) => (
                  <div key={offer.id} className="bg-zinc-900/60 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between group">
                    <div>
                    ...
                    <button onClick={() => handleMatch(offer.id, offer.stake_amount)} className="bg-white text-black font-black px-6 py-3 rounded-xl text-[10px] uppercase">Match {offer.stake_amount}</button>
                  </div>
                ))}
              </div>
            </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredPools.map(pool => (
                <div key={pool.id} className="bg-zinc-900/20 border border-zinc-800 rounded-3xl p-7 flex flex-col h-full group hover:border-[#D4AF37]/50">
                  <h3 className="text-xl font-black mb-3 uppercase group-hover:text-[#FFD700]">{pool.title}</h3>
                  <p className="text-zinc-500 text-sm mb-8 line-clamp-2">{pool.description}</p>
                  <div className="mt-auto space-y-2">
                    <button onClick={() => setLocation(`/pool/${pool.id}`)} className="w-full bg-zinc-800 text-white font-black py-3 rounded-2xl text-[10px] uppercase">Join Global</button>
                    <button onClick={() => { setSelectedPool(pool); setIsModalOpen(true); }} className="w-full bg-gradient-to-br from-[#D4AF37] to-[#FFD700] text-black font-black py-3 rounded-2xl text-[10px] uppercase">Wager a Friend</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            {myWagers.length === 0 ? (
              <div className="py-20 text-center border border-dashed border-zinc-800 rounded-3xl text-zinc-600 uppercase text-[10px] font-black">No active wagers yet.</div>
            ) : (
              myWagers.map((wager) => (
                <div key={wager.id} className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-2xl flex justify-between items-center">
                  <div>
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded mb-2 inline-block ${wager.status === 'matched' ? 'bg-green-500/10 text-green-400' : 'bg-zinc-800 text-zinc-500'}`}>{wager.status}</span>
                  ...
                  <div className="text-right">
                    <p className="text-[9px] uppercase text-zinc-500 font-black">Predicted</p>
      ...
      {isModalOpen && selectedPool && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-[2.5rem] p-8 relative shadow-2xl">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-zinc-500"><X /></button>
            <h3 className="text-xl font-black uppercase italic mb-8">Create Challenge</h3>
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setSideSelection('Yes')} className={`py-4 rounded-xl font-black text-xs transition-all ${sideSelection === 'Yes' ? 'bg-green-500 text-black' : 'bg-zinc-800 text-zinc-500'}`}>YES</button>
                <button onClick={() => setSideSelection('No')} className={`py-4 rounded-xl font-black text-xs transition-all ${sideSelection === 'No' ? 'bg-red-500 text-black' : 'bg-zinc-800 text-zinc-500'}`}>NO</button>
              </div>
              <input type="number" placeholder="Stake Amount (KSh)" className="w-full bg-black border border-zinc-800 rounded-2xl p-5 text-xl font-black text-[#D4AF37] outline-none" value={stakeInput || ''} onChange={(e) => setStakeInput(Number(e.target.value))} />
              <button onClick={handleCreateOffer} className="w-full bg-gradient-to-br from-[#D4AF37] to-[#FFD700] text-black font-black py-5 rounded-2xl uppercase text-xs">Post Challenge</button>
            </div>
          </div>
        </div>
      )}
